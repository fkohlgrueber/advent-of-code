...
impl Challenge for Day {
    type Input = HashMap<i32, [i32; 60]>;

    fn parse(input: String) -> Self::Input {
        // calc sleep history per guard
    ...
    fn part_1(input: Self::Input) -> String {
        ...
        let res = input[&max_id].iter().enumerate().max_by_key(|x| x.1).unwrap();

        (max_id * res.0 as i32).to_string()
    }
    
    fn part_2(input: Self::Input) -> String {
        let res = input.iter().map(get_max).max_by_key(|x| x.2).unwrap();
...
mod tests {
    ...
    #[test]
    fn test_part_1() {
        Day::test_part_1("[1518-11-01 00:00] Guard #10 begins shift\n\
    ...
    #[test]
    fn test_part_2() {
        Day::test_part_2("[1518-11-01 00:00] Guard #10 begins shift\n\
                          [1518-11-01 00:05] falls asleep\n\
                          [1518-11-01 00:25] wakes up\n\
                          [1518-11-01 00:30] falls asleep\n\
                          [1518-11-01 00:55] wakes up\n\
